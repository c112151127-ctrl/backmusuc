...
| 像素風偽 3D / 2.5D | `WorldBackdrop.gd`、Y ordering、PNG 建築、NPC、障礙物與地表層次提供俯視 2.5D 感。 |
| 村莊據點 | `Village.gd` 配置鍛造、合成、補給、改裝、存檔、拆解、出口與 NPC。互動提示改為靠近才顯示，降低畫面混亂。 |
| 冒險公會 | `Guild.gd` 提供任務接取、獎勵交付、出城入口與公會 NPC。 |
| 野外探索 | `Wasteland.gd` 使用大型地圖、seed、資源點、事件點、敵人、障礙物與邊界，支援可重複探索。 |
...
| 人物裝備介面 | `HUD.gd` 新增 `InventoryPanel` / `equipment_panel`，顯示目前左鍵動作、近戰武器、遠程武器、護甲、工具、加成、任務與背包。 |
...
| 敵人類型 | `data/enemies/enemies.json` 定義六種敵人；`PixelArtFactory.gd` 產出不同輪廓的敵人圖像。 |
| 敵人上限與投射物池 | `Wasteland.gd` 生成 30 名敵人；`ProjectilePool.gd` 控制投射物池，維持壓力場景可測。 |
| 資源、裝備、配方 | `equipment.json`、`recipes.json`、`InventorySystem.gd` 與 `GameState.gd` 支援資源堆疊、合成與裝備顯示。 |
| 存讀檔與 checksum | `SaveManager.gd` 使用 `user://save_game.json`，包含 Base64 payload 與 SHA-256 checksum。 |
| 視覺素材 | `RuntimeAssetLoader.gd` 載入 PNG/WAV；`PixelAssetBaker.tscn` 可重新產生像素素材。 |
...
| 視覺 Review | `VisualReviewRunner.tscn` 輸出村莊、公會、野外截圖，供人工確認畫面是否過亂或重疊。 |

## 剩餘差距

- 目前已達可玩 vertical slice，但若以正式上市品質為目標，仍需專職美術重畫高品質角色、建築、敵人與 tileset。
- 裝備介面尚未完成正式 RPG 換裝操作，只先提供可讀的裝備與背包資訊。
- 野外已加入大型地圖、障礙、事件與資源，但還需要更多非戰鬥玩法與場景變化。
