...
| 敵人類型 | `data/enemies/enemies.json` 定義六種普通敵人與一名 Boss；`PixelArtFactory.gd` 產出不同輪廓的敵人圖像。 |
...
| 資源、裝備、配方 | `equipment.json`、`recipes.json`、`InventorySystem.gd` 與 `GameState.gd` 支援資源堆疊、合成與裝備顯示。掉落物使用 `recycler_item_icons.png`，不再使用棋盤格。 |
...
| 視覺 Review | `VisualReviewRunner.tscn` 輸出村莊、公會、野外與野外 Boss 區截圖，供人工確認畫面是否過亂或重疊。 |
