...
| SRS / 設計要求 | 目前證據 |
| --- | --- |
| Godot 4.x 單機 2D Roguelike | `project.godot` 使用 Godot 4.6，主場景進入村莊/野外/公會流程 |
| PC WASD + 滑鼠操作 | `GameState._ensure_input_actions()` 註冊 WASD、滑鼠、攻擊、互動、背包、存讀檔 |
| Android 觸控 UI 基礎 | `scenes/ui/HUD.gd` 提供近戰、射擊、互動、背包、存檔按鈕 |
...
| 可玩性驗證 | `scenes/tests/ValidationRunner.tscn` 自動驗證資料、場景、存檔、裝備循環、近戰擊殺、遠程射擊、戰鬥後存讀檔與回村狀態，可用 `--scene` 命令執行 |
