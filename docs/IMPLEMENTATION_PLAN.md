...
本輪更新的重點是降低畫面混亂、補強 PC 操作邏輯、建立對話框、加入裝備檢視、改善敵人與掉落物樣式、加入 Boss、擴大野外地圖、增加邊界與障礙物，讓畫面更接近參考圖的 2.5D 廢土據點與野外探索感。
...
7. 野外地圖要比村莊大很多，具備邊界、障礙物、資源點、事件點、敵人、Boss 與背景差異。
8. 掉落物必須有可辨識圖示，不可再使用棋盤格或測試色塊。
...
- `scenes/levels/wasteland/Wasteland.gd`：大型野外、敵人、Boss、事件、資源、冒險區域、邊界。
- `scripts/systems/WorldBackdrop.gd`：2.5D 地表、路徑、裂痕、汙染斑塊。
- `scripts/utils/PixelArtFactory.gd`：敵人、Boss、掉落物、地表與 runtime 美術生成。
- `scripts/components/Pickup.gd`：掉落物 atlas 載入、浮動效果、撿取通知。
- `scripts/autoload/SceneRouter.gd`：場景切換延後到安全時機，避免 physics callback 直接移除 CollisionObject。
...
6. 前往公會接委託，再進入野外。
7. 在野外避開障礙、擊倒敵人、觸發事件、撿資源，逐步接近晶化裂隙 Boss 區。
8. 回村合成、強化、補給與存檔。
...
- 七格 atlas：腐爛掠食者、毒脊爬行獸、砲囊爆裂者、巨噬腐肉獸、腐化飛行體、汙染機械體、廢土巨像 Boss。
- 每種敵人需有不同輪廓、顏色重點與攻擊辨識點，不再使用單純色塊。

掉落物圖像：

- 檔案：`assets/sprites/items/recycler_item_icons.png`
- 四種資源：廢鐵、彈藥、異變核心、汙染晶核。
- 掉落物在場景中會有輕微浮動效果，讓玩家知道可以撿取。

冒險區域：

- 廢鐵公路：偏灰橘，障礙與廢車密集。
- 毒沼邊界：偏綠，呈現污染與快速敵人威脅。
- 晶化裂隙：偏紫綠，作為高價資源與 Boss 區域。
...
6. `VisualReviewRunner.tscn` 必須產出村莊、公會、野外與野外 Boss 區截圖，確認文字不大量重疊、NPC 提示只在靠近時顯示。
...
- 野外已擴大並有障礙、事件、冒險區域與 Boss，但還可以加入更多非戰鬥玩法，例如修復裝置、護送、回收路線解謎、天氣與夜晚威脅。
