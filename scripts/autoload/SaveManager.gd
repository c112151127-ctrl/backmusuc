...
func save_game(show_notice := true) -> bool:
	...
	if file == null:
		if show_notice:
			GameState.notify("存檔失敗：無法寫入本機存檔。")
		return false
	file.store_string(JSON.stringify(wrapped, "\t"))
	if show_notice:
		GameState.notify("已儲存目前進度。")
	return true
